...
import { customResponse } from 'src/formatResponse';
...
export class IssueService {
  ...
  async create(createIssueDto: CreateIssueDto): Promise<Issue | any> {
    ...
    if (existingIssue) {
      throw new ConflictException(
        'An issue with the same title and description already exists.',
      );
    }
  ...
  async findAll(page: number = 1, limit: number = 5): Promise<any> {
    ...
    return customResponse(list, 'Issues fetched successfully', true, {
      total,
      page,
      limit,
  ...
  async getIssueById(issueId: number): Promise<Issue[] | any> {
    const getIssue = await this.issueRepository.find({
      where: { id: issueId },
    });
    return {
      data: getIssue,
